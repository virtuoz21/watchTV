...
function ActorComponent() {
  ...
  actorData.forEach((actor) => {
    const existingActor = processedActorData.find(
      (a) => a.person.id === actor.person.id
    );
              ...
              <Grid item xs={4} key={actor.person.id}>
                <div className="actor_card">
                  ...
                  <div className="actor_info">
                    ...
                    <span className="As">
                      {actor.characterNames?.slice(0, 5).join(", ")}
                      {actor.characterNames &&
                        actor.characterNames.length > 5 &&
                        " etc"}
                    </span>
                  </div>
                  <Link className="actor_btn" to={`/actor/${actor.person.id}`}>
                    Show More
                  </Link>
                </div>
              </Grid>
